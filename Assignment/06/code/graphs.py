...
with open('datainit','r') as f:
    for l in f:
        a = l.split('\t')
        x.append(a[0])
        y.append(a[1].rstrip())

it = plt.scatter(y,x,color=[(0,0.5,1)])
...
with open('centerInit','r') as f:
    for l in f:
        a = l.split('\t')
        x.append(a[0])
        y.append(a[1].rstrip())

cit = plt.scatter(y,x,color=[(1,0.5,0)])


plt.autoscale()
plt.legend((dt,ct,it,cit),
           ('Final Sensor Poses', 'Final Robot Pose','Initial Sensor Poses', 'Initial Robot Pose'),
           scatterpoints=1,
           loc='lower left',
           ncol=1,
           fontsize=8)
